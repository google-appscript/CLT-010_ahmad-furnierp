      ...
      <table className="w-full text-sm">
        <thead className="border-b bg-muted/40">
          <tr>
            ...
            <th className="px-4 py-2 text-right font-medium">Total Faktur</th>
          ...
          {dengan.map(({ p, total }) => (
            <tr key={p.id} className="border-b">
              ...
              <td className="px-4 py-1.5">
                <Badge variant={VARIAN[p.status]}>{LABEL_STATUS_PENJUALAN[p.status]}</Badge>
              </td>
              <td className="px-4 py-1.5 text-right">
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/penjualan/pesanan/${p.id}`}>Buka</Link>
                </Button>
              </td>
            </tr>
          ))}
